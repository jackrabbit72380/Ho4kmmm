(function () {
    ...
    var jsonQuery = null;
    var screens = {};
    let screenEventMap = {}; // eventName : [ screen ]
    ...
    ui.notify = function (event, data, broadcast, fromDew) {
        $.each(screens, function (id, screen) {
            if (broadcast || screen.state === ScreenState.VISIBLE) {
                if(screenEventMap[event] != null && screenEventMap[event].includes(screen)) //check that this screen is registered to receive this event
    ...
    ui.createScreen = function (data) {
        ...
        var screen = {
            ...
            set state(newState) {
                ...
                dew.callMethod('screenState', { screenId: this.id, state: newState });
            }
        };
    ...
    ui.dialog = function (data, screen) {
        ui.notify('dialog', Object.assign({}, data, { screen: screen?.id }), true, false);
    };
    ...
    window.addEventListener("message", function (event) {
        ...
        switch (eventData.message) {
            case "registerEvent":
                var eventName = (typeof data.eventName === "string") ? data.eventName : null;
                if(eventName != null && screen != null){
                    if(screenEventMap[eventName] == null){
                        screenEventMap[eventName] = []; //init array for this event
                    }
                    else if(screenEventMap[eventName].some((map_screen) => map_screen == screen)){
                        //we are already registered
                        break;
                    }
                    screenEventMap[eventName].push(screen);
                }
                break;
            case "show":
            ...
            case "toast":
                ...
                break;
            case "dialog":
                ui.dialog(data, findScreenFromWindow(event.source));
                break;
            case "dialogResult":
                if(data.screen) {
                    notifyScreen(findScreen(data.screen), 'dialogResult', data);
                }
                break;
			default:
				var message = (typeof eventData.message === "string") ? eventData.message : (screen ? screen.id : null);
