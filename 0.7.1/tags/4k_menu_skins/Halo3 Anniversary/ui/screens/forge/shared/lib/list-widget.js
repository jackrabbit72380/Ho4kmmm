dew.makeListWidget = function (_elem, settings) {
        ...
        dew.input.on('scroll', function (type, axis, direction) {
            ...
            if (axis == 0) {
                if (type < 2)
                    ...
                    dew.input.playSound(dew.input.Sounds.LeftTrigger + (direction > 0 ? 1 : 0));
        ...
        dew.input.on('action', function ({ action }) {
            ...
            switch (action) {
                case dew.input.Actions.A:
                    if (_currentNode) {
                        dew.input.playSound(dew.input.Sounds.A);
    ...
    function navigate(count) {
        ...
        if (oldIndex != -1 && newIndex != -1 && oldIndex != newIndex) {
            dew.input.playSound(dew.input.Sounds.VerticalNavigation);
        }
    }
