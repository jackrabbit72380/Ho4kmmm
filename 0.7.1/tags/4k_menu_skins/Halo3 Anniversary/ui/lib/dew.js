...
(function () {
    ...
    function registerEvent(name, callback) {
        ...
        //tell ui.js we want this event to route to us
        postUiMessage("registerEvent", {
            eventName: name
        });
        window.addEventListener("message", proxy, false);
        return proxy;
    }
    ...
    dew.dialog = function (template, data) {
        return new Promise((resolve, reject) => {
            // generate a decently unique value to use as the id
            let id = Math.random().toString(36).substr(2, 9);
            let params = { id, template, data };
   
            // add a temporary window message listener to capture the result
            window.addEventListener('message', function listener(message) {
                if (message.data.event === 'dialogResult' || message.data.message === 'dialogResult') {
                    // make sure this is the result we want
                    if (message.data.data.requestId !== params.id)
                        return;

                    // remove the listener
                    window.removeEventListener('message', listener);
                    // resolve the promise with the dialog result
                    resolve(message.data.data.result);
                }
            }, false);
            // forward the request to the main frame
            postUiMessage('dialog', params);
        });
    };
	
    dew.closeDialog = function(dialogName) {
        let params = { data: dialogName }
        postUiMessage("dialogClose", params);
    }

    dew.notifyDialogResult = function (requestId, screen, result) {
        postUiMessage('dialogResult', { requestId, screen, result });
    };

	const uiSounds = {
		error: 0,
		vertical_navigation: 1,
		horizontal_navigation: 2,
		a_button: 3,
		b_button: 4,
		x_button: 5,
		y_button: 6,
		...
		timer_sound_zero: 14,
		alt_timer_sound: 15,
		alt_timer_sound_zero: 16,
		matchmaking_advance_sound: 17,
		rank_up: 18,
		matchmaking_party_up_sound: 19
	};
	// Plays a ui sound by string
	dew.playSound = function(name) {

		if(!(name in uiSounds)) {
			console.error(`ui sound not found '${name}'`);
			return;
		}

		dew.command(`Game.PlayUiSound ${uiSounds[name]}`);
	}

    // Disable text selection by default since most screens won't want it
    window.addEventListener("load", async function (event) {
        document.body.style["-webkit-user-select"] = "none";
        document.body.toggleAttribute('gpu',  await dew.command('cef_gpu_enable') == '1');
    });
    ...
    /**
     * (ASYNCHRONOUS) Retrieves the minimum compatible version of ElDewrito.
     *
     * @returns {DewPromise<string>} - A promise for the version string.
     */
    dew.getVersionCompat = async function () {
        return dew.command("Game.VersionCompat");
    }
     ...
     * If the command does not run successfully, the promise will be rejected with a [DewErrorCode.COMMAND_FAILED]{@link DewErrorCode} error.
     *
     * @param {object|string} variables - The variables to set e.g { "Player.Name": 'John', "Player.ServiceTag": "117" }
     * @param {object} [options] - Additional options that control how the command should run.
     * @param {boolean} [options.save] - If set to true, the variables will be saved immediately.
     * @returns {DewPromise} A promise that gets resolved once the variables have been set
     */
    dew.setVariables = function (variables, options) {
        return dew.callMethod("setVariables", function () {
            options = options || {};
            return {
                variables: variables,
                save: (typeof options.save === "boolean") ? options.save : false
            };
        });
    }
     ...
     * If the command does not run successfully, the promise will be rejected with a [DewErrorCode.COMMAND_FAILED]{@link DewErrorCode} error.
     *
     * @param {string} variables - The variable to set e.g "Player.Name"
     * @param {string} value - The new value for the variable 
     * @param {object} [options] - Additional options that control how the command should run.
     * @param {boolean} [options.save] - If set to true, the variable will be saved immediately.
     * @returns {DewPromise} A promise that gets resolved once the variables have been set
     */
    dew.setVariable = function (variable, value, options) {
        return dew.callMethod("setVariables", function () {
            options = options || {};
            return {
    ...
    /**
     * (ASYNCHRONOUS) Requests blur for a layer.
     *
     * If the command does not run successfully, the promise will be rejected with a [DewErrorCode.COMMAND_FAILED]{@link DewErrorCode} error.
     * The error message will be the command output.
     *
     * @param {string} name - The name of the layer requesting blur for.
     * @param {integer} name - The speed at which the game will blur or unblur.
     * @param {boolean} name - True/false for whether the screen wants blur or not.
     * @returns {DewPromise<string>} A promise for the command output.
     */
    dew.GameBlur = function (layer, enabled, speed = 6) {		
        dew.command('game.FullscreenBlurTime ' + speed + 'f');
        return dew.command('Game.FullScreenBlurLayer ' + layer + ' ' + (enabled ? '1' : '0'));
    }
	
	 /**
     * (ASYNCHRONOUS) Gets an endpoint from dewrito.json.
     *
     * If the command does not run successfully, the promise will be rejected with a [DewErrorCode.COMMAND_FAILED]{@link DewErrorCode} error.
     * The error message will be the command output.
     *
     ...
     * @returns {DewPromise<string>} A promise for the command output.
     */
    dew.getEndpoint = function (name) {		
		return dew.command('Server.GetEndpoint ' + name);
    }
    ...
    */
     ...
     * @returns {EventCallback} The callback proxied event handler
     * @see event:show
     * @see event:hide
     * @see event:pong
     * @see event:console
    ...
    dew.on = function (event, callback) {
        return registerEvent(event, callback);
    }

    /**
     * Remove a dew event listener
     * @param {EventCallback} token - The event handler returned by dew.on
     */
    dew.off = function(token) {
        window.removeEventListener("message", token);
    }

    dew.getAssetUrl = function(path, options) {
        return dew.callMethod('getAssetUrl', { ...options, path, });
    }
