dew.makeColorPicker = function(elem) {
    ...
    function attachEventHandlers() {
        ...
        _hexInputElement.addEventListener('input', function(e) {
            ...
            _hexInputElement.value = value;
			
            var hsv = ColorUtil.hexToHsv(value.substr(1));
            if(hsv)
    ...
    function render(prevState, state) {
        if (!_prevState) {
            ...
            `
                <div class ="ed-color-picker">
                    ...
                    <div class="ed-color-picker__detail">
                        <div class="ed-color-picker_rgb_output">
                            <div style="display:inline-block;width:6ch;">R: 0</div>
                            <div style="display:inline-block;width:6ch;">G: 0</div>
                            <div style="display:inline-block;width:6ch;">B: 0</div>
                        </div>
                        <div>HEX: <input id="ed-color-picker_hex_input" spellcheck="false" maxlength="7" type="text" class='ed-color-picker__detail_hex-input' /></div>
                    </div>
                </div>
            `;
            ...
            _outputElements = _container.querySelectorAll('.ed-color-picker_rgb_output >div');           
        ...
        if (!_prevState.selectedColor || _state.selectedColor != _prevState.selectedColor) {
            ...
            let value = `#${leftPad(ColorUtil.rgbToHex(rgb[0],rgb[1],rgb[2]), 6, '0')}`;
            value = value.toUpperCase();
            _hexInputElement.value = value;
        }
    }
